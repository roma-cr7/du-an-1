...
/**
 * Hàm render một nhóm sản phẩm vào container
 * @param {string} containerId - id của div chứa sản phẩm
 * @param {number} minId - ID nhỏ nhất
 * @param {number} maxId - ID lớn nhất
 */
function renderProducts(containerId, minId, maxId) {
  const container = document.getElementById(containerId);
  if (!container) return console.error(`Không tìm thấy #${containerId}`);
  container.innerHTML = "";
...
// Khi DOM sẵn sàng, render cả 3 khối
window.addEventListener("DOMContentLoaded", () => {
  renderProducts("feature-samsung", 1, 3);
  renderProducts("feature-apple",   7, 9);
  renderProducts("feature-xiaomi", 13, 15);
});