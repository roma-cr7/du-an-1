...
function displayXiaomiProducts() {
  const container = document.getElementById("xiaomi-products");
  container.innerHTML = "";
  //Lọc các sản phẩm Samsung (id 13 - 18)
  const xiaomiProducts = products.filter(p => p.id >= 13 && p.id <= 18);
...
window.onload = displayXiaomiProducts;
